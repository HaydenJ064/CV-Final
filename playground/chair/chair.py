...
def combine_coco_instances(annotation_path):
    ...
    val_instances = json.load(
        open("%s/instances_%s2017.json" % (annotation_path, "val"))
    )
    train_instances = json.load(
        open("%s/instances_%s2017.json" % (annotation_path, "train"))
    )
    all_instances = {
...
class CHAIR(object):
    ...
    def caption_to_words(self, caption):
        ...
        idxs = [
            idx for idx, word in enumerate(words) if word in set(self.mscoco_objects)
        ]
    ...
    def compute_chair(self, cap_file, image_id_key, caption_key):
        ...
        for i in tqdm.trange(len(caps)):
            ...
            if len(gt_objects) > 0:
                ...
                if len(node_words) == 0:
                    ...
                    cap_dict["metrics"]["Precision"] = len(recall_gt_objects) / len(
                        set(node_words)
                    )
                if (
                    cap_dict["metrics"]["Precision"] + cap_dict["metrics"]["Recall"]
                ) == 0:
                    cap_dict["metrics"]["F1"] = 0.0
                else:
                    cap_dict["metrics"]["F1"] = (
                        2
                        * (
                            cap_dict["metrics"]["Recall"]
                            * cap_dict["metrics"]["Precision"]
                        )
                        / (
                            cap_dict["metrics"]["Precision"]
                            + cap_dict["metrics"]["Recall"]
        ...
        chair_i = (
            hallucinated_word_count / coco_word_count if coco_word_count != 0 else 0.0
        )
        # add
        recall = num_recall_gt_objects / num_gt_objects if num_gt_objects != 0 else 0.0
        precision = (
            num_recall_gt_objects / num_generated_objects
